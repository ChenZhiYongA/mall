...
public class UmsAdminServiceImpl implements UmsAdminService {
    ...
    public Map login(String username, String password) {
        ...
        try {
            ...
            //通过用户名称查询用户信息（获取审核字段）
            UmsAdmin byUserName = adminMapper.findByUserName(username);
            if ("0".equals(byUserName.getShenghe())) {
                token = "-1";
                throw new BadCredentialsException("无权限登录");
            }
...
//            updateLoginTimeByUsername(username);
    ...
    @Override
    public Boolean xiugaishenhe(Long id) {
        //调方法查询该数据的审核状态
        String shenghe = adminMapper.selectByPrimaryKey(id).getShenghe();
        if ("1".equals(shenghe)) {
            //调用方法修改数据
            adminMapper.updateshenhe(id, "0");
        } else {
            adminMapper.updateshenhe(id, "1");
        }
